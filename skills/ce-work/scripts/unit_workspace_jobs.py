    ...
    with locked_manifest(args.run_id) as doc:
        ...
        if existing:
            ...
            if existing.get("state") == "cleaned" or existing.get("cleanup"):
                raise Operational("REFUSED", "cleaned unit cannot be prepared again under the same run/unit identity")
        ...
        if existing and existing["workspace"].get("registered"):
            ...
            return "PREPARED", {
                "unit_id": uid, "workspace": workspace, "result_dir": os.path.join(unit_root, "result"),
...
def validate_runner_contract(run_id: str, unit: dict, meta: dict) -> None:
    ...
    if (
        ...
        or meta.get("label") != unit_id
        or meta.get("input_digest") != unit["packet_digest"]
        or not isinstance(meta.get("result_path"), str)
        or os.path.abspath(meta["result_path"]) != expected_result_file
    ):
