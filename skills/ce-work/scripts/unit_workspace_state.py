...
def attempt_authorization(
    ...
    if not isinstance(binding, dict) or not isinstance(egress, dict):
        raise Operational("BLOCKED", "run binding or egress sanction is malformed")
    route = egress.get("route")
    contract = ROUTE_CONTRACTS.get(route)
    if not contract:
        raise Operational("BLOCKED", "egress sanction does not name a supported fixed route")
    if binding.get("target") != contract["target"]:
        raise Operational("BLOCKED", "binding target does not match the sanctioned fixed route")
    intermediaries = egress.get("intermediaries")
    if intermediaries != contract["intermediaries"]:
        raise Operational("BLOCKED", "egress intermediaries do not match the fixed route")
    model = binding.get("model")
    if model is not None and (not isinstance(model, str) or not model):
        raise Operational("BLOCKED", "binding model must be null or a non-empty string")
    ...
    if not isinstance(restrictions, list) or not all(isinstance(item, str) for item in restrictions):
        raise Operational("BLOCKED", "egress restrictions must be a string list")
    return {
        "schema_version": 1,
        "run_id": doc["run_id"],
        "unit_id": unit_id,
