...
Run this protocol from the host checkout for one ready unit at a time. Resolve each bundled script from this skill's directory. The host remains the orchestrator throughout; the external CLI is only the bounded author.

1. **Resolve, preflight, and sanction.** Apply `current-turn directive > typed caller binding > per-checkout configuration > native`, collapse a same-host default to native, verify the fixed adapter and caller restrictions, then disclose the sanction source, route/intermediaries, material exposed, and restriction posture. A preflight-unavailable `prefer` route continues natively with requested-versus-actual disclosure. A preflight-unavailable `require` route asks only in an interactive standalone run; automatic and headless runs return a blocker without starting native work.
2. **Establish clean canonical state.** The selected plan may be the only dirty path. Initialize the private run with `unit-workspace.py` `init`, then use `unit-workspace.py` `checkpoint-plan` for that exact plan-only case and disclose the resulting host checkpoint. That exact plan-only state is checkpointable, not a route blocker; only checkpoint failure or any unrelated dirt makes the route unavailable. Re-read canonical repository, branch, HEAD, plan digest, and cleanliness after a checkpoint.
3. **Prepare one bounded unit packet.** Include only the Goal Capsule, Definition of Done, active unit, relevant Verification Contract rows, cited R/F/AE/KTD excerpts, inherited restrictions, expected files, evidence strategy, and explicit exclusions. Do not expose the whole plan by default. Call `unit-workspace.py` `prepare` with the packet source path, unit id, recorded base, dependencies, wave fields, and the route-qualified activity posture. Use only the returned controller-owned `packet_path` and computed `packet_digest` for dispatch; never substitute the caller's source path or a caller-computed digest.
4. **Start one fixed author.** The controller-issued authorization schema binds `run_id`, `unit_id`, and `attempt_id` to the fixed route/model/intermediary and packet contract. Call `peer-job-runner.py` `start --no-sweep` with skill `ce-work`, the same run/unit label, a two-hour hard cap, and idle disabled only when the route receipt lacks trustworthy activity. Its production worker command is `cross-model-work.sh <authorization_path> <workspace> <unit-packet> <expected-packet-sha256> <result-dir>`; pass only the controller-returned `authorization_path`, never a caller-supplied route string or ambient model override. Before prompt construction or external CLI start, the adapter must obtain controller `authorize-dispatch` success by calling `unit-workspace.py` `authorize-dispatch`. That success binds the authorization digest, workspace, packet path and digest, and result directory to the exact manifest attempt while revalidating the controller-owned exact route, model, and intermediary contract. A missing or refused handshake, including hand-authored or cross-attempt authorization, refuses egress. Model pins come only from the authorized projection. The adapter's `--emit-adapter` mode remains introspection only and never authorizes production dispatch. Capture the returned job id immediately with `unit-workspace.py` `record-job`, which revalidates the same artifact and exact argv. Never hold one host tool call open for the external runtime and never let the adapter select another recipient.
5. **Observe without steering.** Interleave bounded runner `status`/`wait` calls with `unit-workspace.py` `sync-job`; report unit, route, elapsed time, latest meaningful activity, activity posture, and terminal state. For a qualified silent terminal-only route, `hard-only` is the normal posture: disable idle timeout, retain the universal hard cap, and never infer failure or fallback merely from absent incremental activity. A live or temporarily unreachable attempt is still authoritative: do not start fallback or duplicate work. Reap only through the explicit controller/runner path and retain its workspace.
6. **Terminalize complete Git output.** On authoritative `done`, call `unit-workspace.py` `terminalize`. Require its pinned synthetic transport commit to have the recorded base as sole parent and the complete final workspace tree; inspect the actual transport diff, changed paths, binary/mode/rename/delete evidence, adapter result, and any scope-expansion request. Unexpected or broadened scope is preserved for host resolution, not applied.
7. **Acquire and preflight canonical integration.** Call `unit-workspace.py` `integration-acquire`, retain its token, then call `unit-workspace.py` `preflight` with only the recorded base or exact allowed same-wave host commits. Reconfirm repository identity, branch, clean index/worktree, HEAD contract, transport ref, and pre-fold snapshot before mutation.
8. **Apply transactionally.** Run `git cherry-pick --no-commit <transport-commit>` in the canonical checkout, then call `unit-workspace.py` `mark-applied`. Inspect the staged and worktree result again; a clean textual apply is not semantic proof. Conflict or scope rejection goes directly to `unit-workspace.py` `restore`.
9. **Verify and commit as host.** Run the unit's authoritative canonical verification from the Verification Contract. Only a pass may call `unit-workspace.py` `mark-verified`, create one normal host-owned canonical commit, and call `unit-workspace.py` `mark-committed`. The worker's commits and prose never satisfy these states.
10. **Restore before any other action on failure.** Apply, scope, verification, or host-commit failure calls `unit-workspace.py` `restore` while the integration token is held. Exact pre-fold equality must be proven and the unit becomes preserved before fallback, retry, or another unit. If restoration cannot be proven, retain the lock, workspace, transport ref, and recovery path and return a blocker.
11. **Clean only after reconciliation.** After `committed`, call `unit-workspace.py` `cleanup`, verify the worktree is unregistered and the exact transport ref removed, then call `unit-workspace.py` `integration-release`. Failed, divergent, timed-out, scope-broadened, and unintegrated attempts remain owner-checked and inspectable; cleanup requires an explicit exact abandonment receipt.
