...
3. **Prepare one bounded unit packet.** Include only the Goal Capsule, Definition of Done, active unit, relevant Verification Contract rows, cited R/F/AE/KTD excerpts, inherited restrictions, expected files, evidence strategy, and explicit exclusions. Persist the packet under the private run with its digest; do not expose the whole plan by default. Call `unit-workspace.py` `prepare` with the unit id, recorded base, packet digest, dependencies, wave fields, and the route-qualified activity posture.
4. **Start one fixed author.** Call `peer-job-runner.py` `start --no-sweep` with skill `ce-work`, the same run/unit label, a two-hour hard cap, and idle disabled only when the route receipt lacks trustworthy activity. Its worker command is `cross-model-work.sh <fixed-route> <workspace> <unit-packet> <result-dir>`. Capture the returned job id immediately with `unit-workspace.py` `record-job`. Never hold one host tool call open for the external runtime and never let the adapter select another recipient.
