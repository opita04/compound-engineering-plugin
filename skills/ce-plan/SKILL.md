...
- Every load-bearing KTD has a concrete `Binds` surface and a checkable `Rule`; when it is inherited, `Provenance` points to a repo-relative path plus stable anchor. No implementation unit silently contradicts the KTD, and any replacement names the inherited decision and explains the change
...
**Load-bearing architectural KTDs.** When a decision constrains multiple components, interfaces, repositories, or implementation units, render the compact architecture spine from `references/plan-sections.md`: `Decision`, `Binds`, `Prevents`, `Rule`, and optional inherited `Provenance`. Keep routine local KTDs lightweight. An inherited decision remains binding unless the plan records an explicit replacement decision with rationale.
