...
- **`pr-snapshot watch`** is that detector — same fetch→diff on an interval, **no agent tokens**, prints one `BABYSIT_WAKE {reason,url,...}` line *only* on an actionable change or a stop condition (`actionable` / `terminal` / `blocked-external` / `needs-human` / `merge-ready` after settle / `max-runtime` / `stop-signal`), then exits.
