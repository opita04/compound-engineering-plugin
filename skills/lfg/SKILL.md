...
2. Invoke the `ce-work` skill with `mode:return-to-caller <plan-path-from-step-1>`.
   ...
   If the transient carrier exists, pass its exact `implementation_engine.{mode,target,model,source}` data beside `mode:return-to-caller` and the plan path as structured caller data; do not render it into the plan path or implementation prompt. If it does not exist, pass no empty carrier and let `ce-work` resolve standing per-checkout configuration. LFG is an automatic, headless caller: it never prompts to weaken a requirement-strength route.
