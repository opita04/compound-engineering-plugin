...
2. Invoke the `ce-work` skill with `mode:return-to-caller <plan-path-from-step-1>` when no transient carrier exists. When one exists, use the exact string-host form `mode:return-to-caller implementation_engine:<compact-json> <plan-path-from-step-1>`.
   ...
   If `behavior_change: true` but `verification_evidence` is missing or too vague to tell how behavior was protected, invoke `ce-work` one more time in recovery mode. Reuse the same `implementation_engine:<compact-json>` carrier when one existed, add `implementation_run:<safe-id>` with the exact `run_id` from the first return, and keep the same plan path. Do not prompt the user and do not alter the plan path or engine carrier; this is resume/evidence reconciliation, not a fresh dispatch. If the first return has no safe non-null `run_id`, stop as blocked instead of attempting discovery or a second implementation. The recovery relies on ce-work's durable reconciliation path to inspect the already-implemented work, fill the missing evidence, and return without reimplementing. If the second return still lacks coherent verification evidence, stop as blocked and report the missing fields instead of continuing to simplify/review/ship.
