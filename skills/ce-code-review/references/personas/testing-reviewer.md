...
- **No proof at a real consumer boundary** -- when changed behavior crosses a wiring or layer boundary, trace it through at least one real consumer: a caller, command, route, adapter, UI surface, or integration seam that users or other components actually exercise. A helper-only test is insufficient when the regression would occur between layers. A focused unit test is sufficient when the changed behavior is self-contained inside the helper and no consumer wiring or contract changed. Ask: "Would this test fail if the changed behavior regressed at the consumer?" If no, report the missing boundary proof.
- **Missing proof across sibling call sites** -- when the change updates a shared contract, method, status, flag, or interface, inspect sibling call sites and consumers that are in scope for that changed contract. Flag tests that prove one updated path while leaving another in-scope sibling unproved; unrelated or explicitly out-of-scope consumers do not need new proof. This is a missing-adoption verification gap, not a claim that the sibling's runtime behavior is wrong; correctness-reviewer owns confirmed stale behavior.
- **Protective tests outside the normal verification path** -- verify that the claimed test is collected and run by the repository's normal verification command. A test that is skipped, quarantined, excluded by configuration, mocked away from the changed interaction, or only runnable through an undocumented one-off command does not protect the change. Require evidence that the normal verification path would fail if the changed behavior regressed.
