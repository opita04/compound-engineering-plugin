...
**Runtime script invocations via the Bash tool — resolve against the project CWD, on every host:** When skill content tells the agent to *execute* a bundled script through the Bash tool, a bare relative path does **not** work — not on Claude Code, Codex, or Cursor. The Bash tool's working directory is the user's **project**, not the skill directory, on all three (empirically confirmed June 2026: each ran a skill's bundled-script command with `pwd` = the project root, never the skill dir). So `bash scripts/my-script.sh` resolves to `<project>/scripts/…`, finds nothing, and the step is silently skipped — a recurring bug class (#764 `ce-worktree`, #811 `ce-code-review`, #898 `ce-compound`). The agentskills.io standard's claim that "the agent runs commands from [the skill directory]" is **not** honored by any of these three harnesses, so do not rely on it. The portable fix is to anchor to where the agent read the SKILL.md:
...
`SKILL_DIR` is a **model-filled** value, not a harness variable: every harness loads SKILL.md from a real absolute path the agent knows, so the skill instructs the agent to set `SKILL_DIR` to that directory. This works identically on Claude Code, Codex, and Cursor (all three verified June 2026) precisely because it depends on no host-specific variable — `SKILL_DIR`, `CLAUDE_SKILL_DIR`, `CODEX_SKILL_DIR`, `AGENT_SKILL_DIR` all come back **unset** as env vars on every host, yet the script runs because the agent supplies the path. This is the production pattern used by widely-installed cross-host skills (e.g. `last30days`). A script that needs its *own* directory (to read a sibling file) derives it from `BASH_SOURCE` — `SKILL_DIR` is the orchestrator's shell var, not exported to the child process.

**`${CLAUDE_SKILL_DIR}` is a Claude-Code-only alternative — prefer the `SKILL_DIR` anchor above for anything multi-host.** It is a SKILL.md *content* substitution (not an env var) that Claude Code replaces with the skill's absolute path before the agent reads the file; it is empty on Codex/Cursor/Gemini. It is deterministic on Claude (the harness fills it, no model judgment), so it remains fine for a Claude-only skill, kept behind an existence guard; `ce-compound`'s `validate-frontmatter.py` is the canonical example.
...
So: a skill's *core* behavior **can** live in a bundled script across hosts — invoke it via the `SKILL_DIR`-from-read-path anchor above, which is verified portable on Claude Code, Codex, and Cursor. You no longer need to avoid bundled scripts for portability; anchor them instead. (The native-Codex / converter facts below are why a *Claude-only* mechanism like `${CLAUDE_SKILL_DIR}` silently misses off-Claude — they are the reason to use the portable anchor, not a reason to avoid scripts.) The older `${CLAUDE_SKILL_DIR}` existence guard still suits an **optional** Claude-only guard script (e.g. `ce-compound`'s `validate-frontmatter.py`) where the `else` branch runs an equivalent inline check. Read-time references (`references/*.md`) still resolve against the skill dir on all targets and need no anchor.
