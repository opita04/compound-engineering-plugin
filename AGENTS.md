...
**`${CLAUDE_SKILL_DIR}` is a Claude-Code-only alternative — prefer the `SKILL_DIR` anchor for anything multi-host.** It is a SKILL.md *content* substitution (not an env var) that Claude Code replaces with the skill's absolute path before the agent reads the file; it is empty on Codex/Cursor/Gemini. It is deterministic on Claude (the harness fills it), so it stays fine for a *Claude-only* skill behind an existence guard — `ce-compound`'s `validate-frontmatter.py` is the canonical example. The **genuine silent skip** is exactly this guarded form off-Claude: when the var is unset, `[ -n "${CLAUDE_SKILL_DIR}" ]` is false and the `then` branch quietly never fires. That is why a Claude-only mechanism misses on Codex/Cursor — the converter does not rewrite these paths, and the plugin also ships as a *native* Codex plugin that loads raw `SKILL.md` verbatim (no converter, no `ce_platforms` filtering), so the SKILL.md content is the only protection both install paths respect. The `SKILL_DIR` anchor sidesteps all of this.
