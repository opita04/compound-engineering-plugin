...
Before any repository material leaves the host, `ce-work` discloses and records the instruction/config source, fixed recipient and intermediaries, bounded unit material exposed, and which restrictions are adapter-enforced versus cooperative. The adapter uses the CLI's existing authentication, receives a minimized environment, and cannot switch recipients, widen scope, push, open a PR, or choose fallback. If a required restriction cannot be enforced, the route is unavailable.
...
The detached job has a two-hour hard cap. An idle cap is enabled only for a qualified route whose activity is trustworthy; silent terminal-only routes rely on the hard cap. Progress reports the run id, active unit/route, elapsed time, latest meaningful activity, activity posture, worker terminal state, integration, verification, commit, cleanup, blockers, and recovery path rather than streaming the full transcript.

Worker output becomes one complete synthetic transport commit, including committed and residual edits, untracked/binary files, deletes, renames, and mode changes. Under an integration lock, the host verifies the canonical checkout is still clean and at an allowed SHA, applies that transport without committing, inspects scope, runs authoritative tests, and creates one host-owned canonical commit. Any apply, scope, verification, or commit failure restores the exact pre-fold checkout before another unit or fallback may start. Unknown canonical movement blocks integration.
